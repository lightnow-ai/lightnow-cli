...
def test_response_error_message_prefers_nested_error_message() -> None:
    ...
    message = response_error_message(response({"error": {"message": "Session expired"}}))
...
def test_response_error_message_falls_back_to_detail_and_title() -> None:
    ...
    assert response_error_message(response({"detail": "Policy denied"})) == "Policy denied"
...
def test_authentication_error_from_response_uses_safe_message() -> None:
    ...
    error = authentication_error_from_response(response({"error": {"code": "invalid_token"}}, status_code=401))
